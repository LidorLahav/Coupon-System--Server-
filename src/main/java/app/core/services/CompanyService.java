...
public class CompanyService extends ClientService {
    ...
    /**
     * First checks if company exists in database by it's email, then validate the
     * password. Finally initialize the filed companyId to be the company id.
     * 
     * @param email
     * @param password
    ...
    /**
     * First checks if there is not another coupon with the same id, after that
     * checks if the coupon title of the @param(coupon) is exists in database, than
     * if not, adds the @param(coupon) of the company to database.
     * 
     * @throws CouponSystemException in case the coupon title of the company already
     *                               exist in the database
     */
    public void addCoupon(Coupon coupon) throws CouponSystemException {
        if (couponRepository.existsById(coupon.getId())) {
            throw new CouponSystemException(
                    "The id of the coupon already exist in the system. Can't add coupon with the same id.");
        }
        Coupon dbCoupon = couponRepository.findFirstByTitle(coupon.getTitle());
        if (dbCoupon != null) {
            throw new CouponSystemException("The coupon title : " + coupon.getTitle() + " of company with id: "
                    + this.company.getId() + " already exist in the database. Can't add coupon with the same title.");
        }
    ...
    /**
     * First checks if the @param(coupon) exists in database, then checks if
     * the @param(coupon) belong to another company. Finally Updates
     * the @param(coupon) properties in the database
     * 
