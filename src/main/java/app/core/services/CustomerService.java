...
public class CustomerService extends ClientService {
    ...
    /**
     * First checks if customer exists in database by it's email, then validate the
     * password. Finally initialize the filed customerId to be the customer id.
     * 
     * @param email
     * @param password
    ...
    /**
     * Adds coupon purchase to database if: 1. The coupon exists in the database 2.
     * The customer does not purchased it before 3. The amount of the coupon is
     * above zero 4. The coupon is still valid Then reduce the amount of the coupon
     * by one.
     * 
     * @throws CouponSystemException
     */
    public void purchaseCoupon(int couponId) throws CouponSystemException {
        ...
        if (couponRepository.existsByIdAndCustomersId(dbCoupon.getId(), customerId)) {
            throw new CouponSystemException("You already purchesed this coupon " + dbCoupon.toString()
                    + " in the past . you can't purches the same coupon more than once");
        }
        if (dbCoupon.getAmount() == 0) {
            throw new CouponSystemException(
                    "The coupon " + dbCoupon.toString() + "  is currently out of stock. Please select another coupon");
        }
        if (dbCoupon.getEndDate().isBefore(LocalDate.now())) {
            throw new CouponSystemException("The expiration date of the coupon : " + dbCoupon.toString()
                    + " has arrived so it is not possible to purchase it. please select another coupon");
        }
